...
const AdminContainer = () => {
  ...
  return (
    <div className="  bg-gray-100 h-screen">
      <div>
        ...
        <button
          ...
          className={`
            ...
            ${index === activeItem ? `text-white ${condtion = item.name}` : `bg-white  text-gray-500 hover:bg-gray-200`} 
          `}
        >
          {item.icon}
          {item.name}
     ...
     {condtion == "Dashboard" ? <Home /> : condtion == "Courses"?  
     <Courses /> :condtion == "Categories"? 
      <Catagory /> :condtion == "Messages"?  <ContactDisplay /> :condtion == "Users"?  <Users /> :condtion == "Enrolled Students"?  <Catagory /> :condtion == "My Account"?<div className=" text-center  h-screen items-center flex justify-center ">  <Link to={"/profile"}> <h1 className="bg-blue-600 hover:bg-white text-xl hover:text-blue-600 hover:border-2 hover:border-blue-600  text-white font-bold p-6 w-[200px] ">Profile <ArrowForward className=" ml-5"/></h1></Link></div>  :condtion == "Log Out"?  <SignOut /> :"" } 
    </div>
  );
};
