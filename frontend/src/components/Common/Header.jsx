...
const Header = () => {
  ...
  return (
    <header className="bg-slate-100 dark:bg-gray-800 py-4 dark:text-white duration-200 relative font-serif">
      <div className="flex items-center justify-between px-4 py-3 mx-auto max-w-screen-xl sm:px-6 lg:px-8">
        <div className="flex items-center">
          <button onClick={toggleMenu} className="block sm:hidden mr-4 z-50">
            {showMenu ? (
        ...
        <div className="flex items-center gap-4">
          {/* Mobile Menu */}
          <div
            className={`sm:hidden fixed top-0 left-0 z-40 bg-white dark:bg-gray-800 p-4 shadow-md w-full h-screen overflow-y-auto transition-transform duration-300 ease-in-out ${
          ...
          >
            ...
            <ul className="flex flex-col gap-6 text-blue-800 dark:text-white">
              {Menu &&
                Menu.map((data) => (
                  <li key={data.id} className="hover:bg-slate-200 p-2 rounded">
                    <Link
                      to={data.link}
                      className="hover:text-blue-800 group font-mono text-lg flex items-center"
                      onClick={toggleMenu}
                    >
          ...
          <div className="relative group hidden sm:block">
            <form onSubmit={handleSearch}>
              <input
                ...
                onChange={handleSearchChange}
                placeholder="Search For Courses"
                className="sm:w-[200px] sm:group-hover:w-[300px] transition-all duration-300 border-2 border-gray-300 px-2 py-1 focus:outline-none focus:border-1 focus:border-primary dark:border-gray-500 dark:bg-gray-800"
              />
              <button type="submit">
